...
public class VerticalStripedCandy extends Candy {
	...
	public VerticalStripedCandy(int color) {
		super(color);
	}

	public VerticalStripedCandy(int color, ImageIcon candyIcon) {
	...
	public void crush() 
	{
		if (this.color != 0)
		{
			...
			{
				if (this.board[this.column][row] != this & this.board[this.column][row].getColor() != 0)
				{
					this.board[this.column][row].crush();
				}
			}
		}
	}
	...
	public void visit(HorizontalStripedCandy other) 
	{
		...
		this.board[this.column][this.row] = other;
		this.crush();
		if (this.row != other.getRow())//if other was in the same row, other already exploded
		{
			other.crush();
		}
	...
	public void visit(VerticalStripedCandy other) 
	{
		...
		{
			if (this.board[col][other.getRow()]!= other)
			{
				this.board[col][other.getRow()].crush();
			}
		}
	...
	public void visit(BagCandy other) 
	{
		...
		this.board[this.column][this.row] = other;
		other.setRow(this.row);
		other.setColumn(this.column);
		other.crush();
		if (this.column > 0)
		{
			for (int row = 0 ; row <= SIZE-1 ; row++)
